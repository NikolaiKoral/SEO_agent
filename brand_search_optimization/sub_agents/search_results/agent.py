...
if not constants.DISABLE_WEB_DRIVER:
    try:
        ...
        # Ensure chromedriver is available or specify path if needed
        driver = selenium.webdriver.Chrome(options=options)
    except Exception as e:
        print(f"Error initializing Selenium WebDriver: {e}")
        # Keep driver as None if initialization fails


def go_to_url(url: str) -> str:
    """Navigates the browser to the given URL."""
    if constants.DISABLE_WEB_DRIVER:
        return "Web driver is disabled. Cannot navigate to URL."
...
def take_screenshot(tool_context: ToolContext) -> dict:
    ...
    if constants.DISABLE_WEB_DRIVER:
        return {"status": "error", "message": "Web driver is disabled. Cannot take screenshot."}
...
# Note: click_at_coordinates seems unused, but let's add the check anyway.
def click_at_coordinates(x: int, y: int) -> str:
    """Clicks at the specified coordinates on the screen."""
    if constants.DISABLE_WEB_DRIVER:
        return "Web driver is disabled. Cannot click at coordinates."
    driver.execute_script(f"window.scrollTo({x}, {y});")
    driver.find_element(By.TAG_NAME, "body").click()
    return f"Clicked at coordinates ({x}, {y})." # Added return statement for consistency


def find_element_with_text(text: str) -> str:
    """Finds an element on the page with the given text."""
    if constants.DISABLE_WEB_DRIVER:
        return "Web driver is disabled. Cannot find element."
    ...
    try:
        ...
        if element:
            ...
            # This case might not be reachable if find_element throws NoSuchElementException
            return "Element not found."
    except selenium.common.exceptions.NoSuchElementException:
        return "Element not found."
    except selenium.common.exceptions.ElementNotInteractableException:
        # Finding doesn't usually raise this, but keeping for safety
        return "Element found but not interactable."


def click_element_with_text(text: str) -> str:
    """Clicks on an element on the page with the given text."""
    if constants.DISABLE_WEB_DRIVER:
        return "Web driver is disabled. Cannot click element."
...
def enter_text_into_element(text_to_enter: str, element_id: str) -> str:
    ...
    if constants.DISABLE_WEB_DRIVER:
        return "Web driver is disabled. Cannot enter text."
    print(
...
def scroll_down_screen() -> str:
    ...
    if constants.DISABLE_WEB_DRIVER:
        return "Web driver is disabled. Cannot scroll."
...
def get_page_source() -> str:
    ...
    if constants.DISABLE_WEB_DRIVER:
        return "Web driver is disabled. Cannot get page source."
    LIMIT = 1000000
